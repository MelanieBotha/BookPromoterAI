...

{
...

    {
...

        {
            ...
            StripeSecretKey = TrimSecret(config["Stripe:SecretKey"]),

            StripePublishableKey = TrimSecret(config["Stripe:PublishableKey"]),

            StripeWebhookSecret = TrimSecret(config["Stripe:WebhookSecret"]),

            PayPalClientId = TrimSecret(config["PayPal:ClientId"]),

            PayPalClientSecret = TrimSecret(config["PayPal:ClientSecret"]),

            PayPalWebhookId = TrimSecret(config["PayPal:WebhookId"]),
    ...
    static string TrimSecret(string? value) =>
