...
static class TikTokPage
{
    public static string Render(AppStoreDb store, PostGenerator generator, string notice = "")
    {
        ...
        var (weekNum, yearNum, currentWeekLabel) = AdWeek.For(DateTime.UtcNow);
        ...
        var tiktokAccount = store.TikTokAccount;
        var tiktokLive = tiktokAccount?.IsLiveConnection == true;
        var autoPostOn = store.TikTokAutoPostEnabled;
        store.EnsureTikTokSchedule();
        var (planLabel, planMax, videosPerWeek, sentThisWeek, generatedThisWeek, _) = store.GetTikTokWeekQuota();
        var planMaxText = planMax is int m ? $"{m} per week" : "unlimited per week";
        var inputMax = planMax ?? 50;
        var connectReturn = Uri.EscapeDataString(SocialConnectHelper.VideosReturnPath);
        var connectHref = $"/social-accounts/connect/TikTok?return={connectReturn}";

        var videoRows = new StringBuilder();
        foreach (var video in videos.Where(v => !(v.AutoGenerated && v.WeekNumber == weekNum && v.WeekYear == yearNum)))
            videoRows.Append(RenderVideoRow(video, tiktokLive, store.IsTikTokConfigured));

        var thisWeekRows = new StringBuilder();
        foreach (var video in thisWeekVideos)
            thisWeekRows.Append(RenderVideoRow(video, tiktokLive, store.IsTikTokConfigured));

        if (thisWeekVideos.Count == 0)
            thisWeekRows.Append("""<p class="muted">No videos for this week yet. Add books with covers, then use <strong>Generate this week's videos</strong>.</p>""");

        var weekActionLabel = thisWeekVideos.Count == 0
            ? "Generate this week's videos"
            : "Regenerate this week's videos";
        var weekActionConfirm = thisWeekVideos.Count == 0
            ? ""
            : "Replace this week's videos with a fresh batch? Existing ones will be removed.";
        ...
        var connectedLabel = tiktokAccount is null
            ? ""
            : H.Encode(string.IsNullOrWhiteSpace(tiktokAccount.DisplayName) ? tiktokAccount.Handle : tiktokAccount.DisplayName);
        var profileLink = tiktokAccount is not null && AuthorFollowLinks.ProfileUrl(tiktokAccount) is string profileUrl
            ? $""" — <a href="{H.Encode(profileUrl)}" target="_blank" rel="noopener">Open profile</a>"""
            : "";

        var removeTikTokButton = tiktokAccount is not null
            ? $"""
                <form method="post" action="/social-accounts/delete/{tiktokAccount.Id}" onsubmit="return confirm('Disconnect TikTok from BookPromoter AI? You can connect again later.');" style="margin:0;display:inline">
            ...
            : $"""
                <section class="panel" style="margin-bottom:1rem">
                    ...
                    {(tiktokAccount is not null
                        ? $"""
                            <p class="muted">
                                Connected as <strong>{connectedLabel}</strong>{profileLink}.
                                {(tiktokLive
                            ...
                            """
                        : """<p class="muted">Connect TikTok to send videos to your inbox from this page (or keep downloading and posting yourself).</p>""")}
                    <div class="row-actions" style="gap:0.75rem;align-items:center;flex-wrap:wrap;margin:0.75rem 0">
                        <a class="button small" style="background:#000" href="{connectHref}">{(tiktokAccount is not null ? "Reconnect TikTok" : "Connect TikTok")}</a>
                        {removeTikTokButton}
                    </div>
                </section>
                <section class="panel" style="margin-bottom:1rem">
                    ...
                    <form method="post" action="/videos/schedule" class="form" style="margin-top:0.75rem">
                        ...
                        <label class="checkbox-row">
                            <input type="checkbox" name="autoPost" value="1" {(autoPostOn ? "checked" : "")}>
                            Auto-send Ready videos to my TikTok inbox (within this week's limit)
                        </label>
    ...
    static string RenderVideoRow(TikTokVideo video, bool tiktokConnected, bool tiktokConfigured)
    {
        ...
        var errorNote = !string.IsNullOrWhiteSpace(video.ErrorMessage)
            && video.Status is TikTokVideoStatuses.Failed or TikTokVideoStatuses.Ready
        ...
        var preview = canDownload
            ? $"""
                ...
                <a class="button small tiktok-download-btn" href="{H.Encode(video.VideoUrl)}" download="{H.Encode(video.BookTitle)}-promo.mp4">Download video</a>
                """
            : video.Status == TikTokVideoStatuses.Rendering
                ? """<div class="tiktok-video-placeholder muted">Rendering 60s video from your book description…</div>"""
                : """<div class="tiktok-video-placeholder muted">No preview</div>""";
        var download = canDownload
            ? $"""<a class="button small" href="{H.Encode(video.VideoUrl)}" download="{H.Encode(video.BookTitle)}-promo.mp4">Download</a>"""
            : "";
        var connectHref = $"/social-accounts/connect/TikTok?return={Uri.EscapeDataString(SocialConnectHelper.VideosReturnPath)}";
        var push = "";
        if (video.Status == TikTokVideoStatuses.Ready && !string.IsNullOrWhiteSpace(video.VideoUrl))
        {
            if (tiktokConnected)
            {
                push = $"""<button type="button" class="button small" style="background:#000" onclick="postToTikTok({video.Id}, this)">Push to TikTok</button>""";
            }
            else if (tiktokConfigured)
            {
                push = $"""<a class="button small" style="background:#000" href="{H.Encode(connectHref)}">Push to TikTok</a>""";
            }
        }
        else if (video.Status == TikTokVideoStatuses.Sent && tiktokConnected && !string.IsNullOrWhiteSpace(video.VideoUrl))
        {
            push = $"""<button type="button" class="button small secondary" onclick="postToTikTok({video.Id}, this)">Push again</button>""";
        }

        var retry = video.Status == TikTokVideoStatuses.Failed && video.AutoGenerated
            ? $"""<button type="button" class="button small secondary" onclick="retryVideo({video.Id}, this)">Retry</button>"""
            : "";
        var regenerate = video.Status is TikTokVideoStatuses.Ready or TikTokVideoStatuses.Sent && video.AutoGenerated
            ? $"""<button type="button" class="button small secondary" onclick="regenerateVideo({video.Id}, this)">Regenerate</button>"""
            : "";

        return $"""
            <article class="book-row tiktok-video-row" id="video-{video.Id}">
                <div>
                ...
                <div class="row-actions">
                    ...
                    {retry}
                    {regenerate}
                    <button type="button" class="danger-button small" onclick="removeVideo({video.Id}, this)">Remove</button>
                </div>
            </article>
            """;
    }

    static string VideosListScript() => """
        <script>
        function videoCsrfFormData() {
            const fd = new FormData();
            const field = document.querySelector('meta[name="csrf-field"]');
            const token = document.querySelector('meta[name="csrf-token"]');
            if (field && token) fd.append(field.content, token.content);
            return fd;
        }
        async function removeVideo(id, btn) {
            ...
            const res = await fetch('/videos/delete/' + id + '?ajax=1', { method: 'POST', body: fd, credentials: 'same-origin' });
            if (!res.ok) { alert('Could not remove video. Refresh and try again.'); return; }
            const row = document.getElementById('video-' + id);
            if (row) row.remove();
            window.scrollTo(0, scrollY);
        }
        async function retryVideo(id, btn) {
            const scrollY = window.scrollY;
            sessionStorage.setItem('videos-scroll', String(scrollY));
            btn.disabled = true;
            const fd = videoCsrfFormData();
        ...
        async function regenerateVideo(id, btn) {
            if (!confirm('Regenerate this video with a fresh narration?')) return;
            const scrollY = window.scrollY;
            sessionStorage.setItem('videos-scroll', String(scrollY));
            btn.disabled = true;
            const fd = videoCsrfFormData();
            try {
                const res = await fetch('/videos/regenerate/' + id + '?ajax=1', { method: 'POST', body: fd, credentials: 'same-origin' });
                const data = await res.json().catch(function () { return {}; });
                if (!res.ok) {
                    btn.disabled = false;
                    alert(data.error || ('Could not regenerate (' + res.status + '). Refresh and try again.'));
                    return;
                }
                location.href = '/videos?retried=1#videos-week';
            } catch (e) {
        ...
        async function regenerateWeek(btn) {
            ...
            const fd = videoCsrfFormData();
            try {
                const res = await fetch('/videos/regenerate-week?ajax=1', { method: 'POST', body: fd, credentials: 'same-origin' });
                const data = await res.json().catch(function () { return {}; });
                if (!res.ok) {
                    btn.disabled = false;
                    alert(data.error || ('Could not generate videos (' + res.status + '). Refresh and try again.'));
                    return;
                }
                location.href = '/videos?regenerated=1&n=' + (data.queued || 0) + '#videos-week';
            } catch (e) {
                btn.disabled = false;
                alert('Could not reach the server. Refresh and try again.');
            }
        ...
        async function postToTikTok(id, btn) {
            if (!confirm('Send this video to your TikTok inbox? You can still download it afterward.')) return;
            const scrollY = window.scrollY;
            sessionStorage.setItem('videos-scroll', String(scrollY));
            btn.disabled = true;
            ...
            const fd = videoCsrfFormData();
            try {
                const res = await fetch('/videos/post/' + id + '?ajax=1', { method: 'POST', body: fd, credentials: 'same-origin' });
                const data = await res.json().catch(function () { return {}; });
                if (!res.ok) {
                    btn.disabled = false;
                    btn.textContent = original || 'Push to TikTok';
                    alert(data.error || ('Could not post (' + res.status + '). Refresh and try again.'));
                    return;
                }
                location.href = '/videos?posted=1#videos-week';
            } catch (e) {
                btn.disabled = false;
                btn.textContent = original || 'Push to TikTok';
                alert('Could not reach the server. Refresh and try again.');
        ...
        (function () {
            ...
            } else if (location.hash === '#videos-week') {
                const el = document.getElementById('videos-week');
