...
static class TikTokPage
{
    public static string Render(AppStoreDb store, PostGenerator generator, string notice = "")
    {
        ...
        foreach (var video in videos.Where(v => !(v.AutoGenerated && v.WeekNumber == weekNum && v.WeekYear == yearNum)))
            videoRows.Append(RenderVideoRow(video, tiktokLive));

        var thisWeekRows = new StringBuilder();
        foreach (var video in thisWeekVideos)
            thisWeekRows.Append(RenderVideoRow(video, tiktokLive));
    ...
    static string RenderVideoRow(TikTokVideo video, bool tiktokConnected)
    {
        ...
        var download = canDownload
            ...
            : "";
        var post = tiktokConnected
            && video.Status == TikTokVideoStatuses.Ready
            && !string.IsNullOrWhiteSpace(video.VideoUrl)
            ? $"""<button type="button" class="button small" style="background:#000" onclick="postToTikTok({video.Id}, this)">Post to TikTok</button>"""
            : "";
