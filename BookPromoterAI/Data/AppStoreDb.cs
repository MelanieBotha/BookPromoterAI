...
class AppStoreDb
{
    ...
    public bool IsBrandFacebookOAuthReady => _settings.IsBrandFacebookOAuthReady;
    ...
    public List<SocialSchedule> Schedules
    {
        get
        {
            ...
            if (uid == 0) return [];
            using var db = Db();
            // TikTok is managed on the Videos page (video inbox), not text Ad Library posts.
    ...
    public List<SocialSchedule> ConnectedAuthorSchedules()
    {
        ...
        return Schedules
            .Where(s => !PostLimits.IsTikTok(s.Platform)
                && accounts.Any(a => PostLimits.PlatformsMatch(a.Platform, s.Platform)))
            .ToList();
    }
    ...
    public void SaveSchedules(List<SocialSchedule> schedules)
    {
        ...
        var orphanSchedules = db.SocialSchedules
            ...
            .AsEnumerable()
            .Where(s => !PostLimits.IsTikTok(s.Platform)
                && !keptPlatforms.Any(p => PostLimits.PlatformsMatch(p, s.Platform)))
            .ToList();
    ...
    public List<TikTokVideo> TikTokVideos
    {
        get
        {
            ...
            return db.TikTokVideos
                .Where(v => v.UserId == uid && v.Status != TikTokVideoStatuses.Removed
                    && (v.VideoKind == "" || v.VideoKind == TikTokVideoKinds.Author))
                .AsNoTracking()
                .OrderByDescending(v => v.WeekYear).ThenByDescending(v => v.WeekNumber).ThenByDescending(v => v.CreatedAt)
                .ToList()
                .Select(ToModel)
                .ToList();
        }
    ...
    public List<TikTokVideo> TikTokVideosThisWeek
    {
        get
        {
            var uid = CurrentUserId();
            if (uid == 0) return [];
            var (week, year, _) = AdWeek.For(DateTime.UtcNow);
            using var db = Db();
            return db.TikTokVideos
                .Where(v => v.UserId == uid && v.WeekNumber == week && v.WeekYear == year
                    && v.Status != TikTokVideoStatuses.Removed
                    && (v.VideoKind == "" || v.VideoKind == TikTokVideoKinds.Author))
                .AsNoTracking()
                .OrderBy(v => v.BookTitle)
                .ToList()
                .Select(ToModel)
                .ToList();
        }
    ...
    public int EnsureWeeklyVideos(PostGenerator generator, string baseUrl, int? userId = null)
    {
        var uid = userId ?? CurrentUserId();
        if (uid == 0) return 0;

        var (currentWeek, currentYear, weekLabel) = AdWeek.For(DateTime.UtcNow);
        var videosPerWeek = EnsureTikTokScheduleForUser(uid).PostsPerWeek;
        if (videosPerWeek <= 0) return 0;

        using var db = Db();
        var books = db.Books.Where(b => b.UserId == uid && b.CoverImageUrl != "").ToList();
        if (books.Count == 0) return 0;

        // Ignore Removed — soft-deleted videos must not block recreating this week's batch.
        var existingBookIds = db.TikTokVideos
            .Where(v => v.UserId == uid && v.WeekNumber == currentWeek && v.WeekYear == currentYear
                && v.AutoGenerated && v.Status != TikTokVideoStatuses.Removed
                && (v.VideoKind == "" || v.VideoKind == TikTokVideoKinds.Author))
            .Select(v => v.BookId)
            .ToHashSet();
        var existingCount = existingBookIds.Count;
        var remainingSlots = Math.Max(0, videosPerWeek - existingCount);
        ...
        foreach (var book in books)
        {
            ...
            db.TikTokVideos.Add(new DbTikTokVideo
            {
                ...
                Title = $"{book.Title} — {weekLabel}",
                Caption = caption,
                NarrationText = narration,
                VideoUrl = "",
                Status = TikTokVideoStatuses.Rendering,
                VideoKind = TikTokVideoKinds.Author,
    ...
    public int RetryFailedWeeklyVideo(int id, PostGenerator generator, string baseUrl = "") =>
        RegenerateWeeklyVideo(id, generator, baseUrl, allowReady: false);

    /// <summary>
    /// Re-queue one auto-generated weekly video (Failed and optionally Ready) with fresh narration.
    /// </summary>
    public int RegenerateWeeklyVideo(int id, PostGenerator generator, string baseUrl, bool allowReady = true)
    {
        var uid = CurrentUserId();
        if (uid == 0) return 0;
        using var db = Db();
        var row = db.TikTokVideos.FirstOrDefault(v =>
            v.Id == id && v.UserId == uid && v.AutoGenerated
            && (v.Status == TikTokVideoStatuses.Failed
                || (allowReady && (v.Status == TikTokVideoStatuses.Ready || v.Status == TikTokVideoStatuses.Sent))));
        if (row is null) return 0;

        var book = db.Books.Include(b => b.Links).FirstOrDefault(b => b.Id == row.BookId && b.UserId == uid);
        if (book is not null)
        {
            ...
            row.Title = $"{book.Title} — {row.WeekLabel}";
        }
    ...
    /// <summary>
    /// Soft-remove this week's auto-generated videos and queue a fresh set (one per book with a cover).
    /// Use after deletions or when you want a new weekly batch.
    /// </summary>
    public int RegenerateThisWeeksVideos(PostGenerator generator, string baseUrl)
    {
        var uid = CurrentUserId();
        if (uid == 0) return 0;
        ...
        using (var db = Db())
        {
            ...
            {
                row.Status = TikTokVideoStatuses.Removed;
                row.VideoUrl = "";
                row.ErrorMessage = null;
            }
            ...
            if (existing.Count > 0 || books.Count > 0)
                db.SaveChanges();
        }

        return EnsureWeeklyVideos(generator, baseUrl, uid);
    }

    public int RequeueFailedWeeklyVideos(PostGenerator generator, int? userId = null)
    {
        var uid = userId ?? CurrentUserId();
        var (week, year, _) = AdWeek.For(DateTime.UtcNow);
        using var db = Db();
        var query = db.TikTokVideos
            .Where(v => v.Status == TikTokVideoStatuses.Failed
                && v.AutoGenerated
                && v.WeekNumber == week
                && v.WeekYear == year);
        if (uid > 0)
            query = query.Where(v => v.UserId == uid);

        var failed = query.ToList();
        if (failed.Count == 0) return 0;

        foreach (var row in failed)
        {
            if (TikTokVideoKinds.IsBrand(row.VideoKind))
            {
                var seed = AppPromoGenerator.WeeklyPromoSeed(DateTime.UtcNow, "TikTok", row.Id);
                row.NarrationText = AppPromoGenerator.BuildAppVideoNarration("", seed);
            }
            else
            {
        ...
        VideoRenderService renderer, string uploadsDir, string appBaseUrl, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var row in pending)
        {
            ...
            if (TikTokVideoKinds.IsBrand(row.VideoKind))
            {
                bookModel = new Book
                {
                    Title = "BookPromoter AI",
                    AuthorName = "BookPromoter AI",
                    Description = row.NarrationText,
            ...
            else
            {
                var book = await db.Books.AsNoTracking()
                    .FirstOrDefaultAsync(b => b.Id == row.BookId && b.UserId == row.UserId, cancellationToken);
                if (book is null)
                {
                    row.Status = TikTokVideoStatuses.Failed;
                    row.ErrorMessage = "Book not found.";
                    continue;
                }
            ...
            var (ok, videoUrl, error) = await renderer.RenderNarratedVideoAsync(
                bookModel, row.NarrationText, uploadsDir, appBaseUrl, cancellationToken);
            if (ok && !string.IsNullOrWhiteSpace(videoUrl))
            {
                row.VideoUrl = videoUrl;
                row.Status = TikTokVideoStatuses.Ready;
                row.ErrorMessage = null;
                rendered++;
            }
            else
            {
                row.Status = TikTokVideoStatuses.Failed;
                row.ErrorMessage = error ?? "Rendering failed.";
            }
        }
    ...
    public async Task RunWeeklyVideoPipelineAsync(
        ...
        TikTokService? tiktok = null, CancellationToken cancellationToken = default)
    {
        ...
        if (tiktok is not null && IsTikTokConfigured)
        {
            await AutoPostReadyTikTokVideosAsync(tiktok, appBaseUrl, cancellationToken);
            await AutoPostReadyBrandTikTokVideosAsync(tiktok, appBaseUrl, cancellationToken);
        }
    }
    ...
    public SocialAccount? BrandTikTokAccount =>
        OwnerSocialAccounts.FirstOrDefault(a => PostLimits.IsTikTok(a.Platform) && a.IsConnected);

    public List<TikTokVideo> BrandTikTokVideos
    {
        get
        {
            if (!IsOwner) return [];
            ...
            using var db = Db();
            return db.TikTokVideos
                .Where(v => v.UserId == uid && v.VideoKind == TikTokVideoKinds.Brand
                    && v.Status != TikTokVideoStatuses.Removed)
                .AsNoTracking()
                .OrderByDescending(v => v.WeekYear).ThenByDescending(v => v.WeekNumber).ThenByDescending(v => v.CreatedAt)
                .ToList()
    ...
    public List<TikTokVideo> BrandTikTokVideosThisWeek
    {
        get
        {
            if (!IsOwner) return [];
            ...
            return db.TikTokVideos
                .Where(v => v.UserId == uid && v.VideoKind == TikTokVideoKinds.Brand
                    && v.WeekNumber == week && v.WeekYear == year
                    && v.Status != TikTokVideoStatuses.Removed)
                .AsNoTracking()
    ...
    public SocialSchedule EnsureBrandTikTokSchedule()
    {
        if (!IsOwner) return new SocialSchedule { Platform = "TikTok", PostsPerWeek = 0 };
        var uid = BrandDataUserId();
        if (uid == 0) return new SocialSchedule { Platform = "TikTok", PostsPerWeek = 0 };
        ...
        db.SocialSchedules.Add(row);
        db.SaveChanges();
        return ToModel(row);
    }

    public string? SaveBrandTikTokVideoSchedule(int videosPerWeek, bool autoPost)
    {
        if (!IsOwner) return "Owner only.";
        var uid = BrandDataUserId();
        if (uid == 0) return "Owner account not found.";
        var clamped = Math.Clamp(videosPerWeek, 0, 7);
        using var db = Db();
        var currentWeek = System.Globalization.ISOWeek.GetWeekOfYear(DateTime.UtcNow);
        ...
        if (schedule is null)
        {
            schedule = new DbSocialSchedule
            {
                UserId = uid,
                Platform = "TikTok",
                PostsPerWeek = clamped,
                RequiresApproval = false,
            ...
            db.SocialSchedules.Add(schedule);
        }
        else
        {
            ...
            schedule.PostsPerWeek = clamped;
            schedule.AutoPostEnabled = autoPost;
        }
    ...
    public (int VideosPerWeek, int SentThisWeek, int GeneratedThisWeek, bool AutoPost) GetBrandTikTokWeekQuota()
    {
        var schedule = EnsureBrandTikTokSchedule();
        var (week, year, _) = AdWeek.For(DateTime.UtcNow);
        var uid = BrandDataUserId();
        using var db = Db();
        var generated = db.TikTokVideos.Count(v =>
            v.UserId == uid && v.VideoKind == TikTokVideoKinds.Brand
            && v.WeekNumber == week && v.WeekYear == year
            && v.AutoGenerated && v.Status != TikTokVideoStatuses.Removed);
        return (schedule.PostsPerWeek, schedule.PostsSentThisWeek, generated, schedule.AutoPostEnabled);
    }

    public int EnsureBrandWeeklyVideos(string baseUrl)
    {
        var uid = BrandDataUserId();
        if (uid == 0) return 0;

        var schedule = EnsureBrandTikTokScheduleForUser(uid);
        if (schedule.PostsPerWeek <= 0) return 0;

        var (currentWeek, currentYear, weekLabel) = AdWeek.For(DateTime.UtcNow);
        using var db = Db();
        var existingCount = db.TikTokVideos.Count(v =>
            v.UserId == uid && v.VideoKind == TikTokVideoKinds.Brand
            && v.WeekNumber == currentWeek && v.WeekYear == currentYear
            && v.AutoGenerated && v.Status != TikTokVideoStatuses.Removed);
        var remaining = Math.Max(0, schedule.PostsPerWeek - existingCount);
        ...
        {
            var seed = AppPromoGenerator.WeeklyPromoSeed(now, "TikTok", existingCount + i);
            var caption = AppPromoGenerator.GeneratePromoPost("TikTok", baseUrl, seed);
            var narration = AppPromoGenerator.BuildAppVideoNarration(baseUrl, seed);
            db.TikTokVideos.Add(new DbTikTokVideo
            {
                UserId = uid,
                BookId = 0,
                BookTitle = "BookPromoter AI",
                Title = $"BookPromoter AI — {weekLabel}" + (remaining > 1 ? $" #{existingCount + i + 1}" : ""),
                Caption = caption,
                NarrationText = narration,
                VideoUrl = "",
                Status = TikTokVideoStatuses.Rendering,
                VideoKind = TikTokVideoKinds.Brand,
                WeekNumber = currentWeek,
                WeekYear = currentYear,
                WeekLabel = weekLabel,
                AutoGenerated = true,
    ...
    SocialSchedule EnsureBrandTikTokScheduleForUser(int userId)
    {
        using var db = Db();
        var existing = db.SocialSchedules.FirstOrDefault(s =>
            s.UserId == userId
    ...
    public int RegenerateBrandThisWeeksVideos(string baseUrl)
    {
        if (!IsOwner) return 0;
        var uid = BrandDataUserId();
        if (uid == 0) return 0;
        ...
        return EnsureBrandWeeklyVideos(baseUrl);
    }

    public int RetryFailedBrandWeeklyVideo(int id, string baseUrl)
    {
        if (!IsOwner) return 0;
        var uid = BrandDataUserId();
        using var db = Db();
        var row = db.TikTokVideos.FirstOrDefault(v =>
            v.Id == id && v.UserId == uid && v.VideoKind == TikTokVideoKinds.Brand
            && v.AutoGenerated && v.Status == TikTokVideoStatuses.Failed);
        if (row is null) return 0;
        var seed = AppPromoGenerator.WeeklyPromoSeed(DateTime.UtcNow, "TikTok", row.Id);
        row.Caption = AppPromoGenerator.GeneratePromoPost("TikTok", baseUrl, seed);
        row.NarrationText = AppPromoGenerator.BuildAppVideoNarration(baseUrl, seed + 17);
        row.CreatedAt = DateTime.UtcNow;
        row.Status = TikTokVideoStatuses.Rendering;
        row.ErrorMessage = null;
        row.VideoUrl = "";
    ...
    public int RegenerateBrandWeeklyVideo(int id, string baseUrl)
    {
        if (!IsOwner) return 0;
        var uid = BrandDataUserId();
        using var db = Db();
        var row = db.TikTokVideos.FirstOrDefault(v =>
            v.Id == id && v.UserId == uid && v.VideoKind == TikTokVideoKinds.Brand
            && v.AutoGenerated
            && (v.Status == TikTokVideoStatuses.Ready || v.Status == TikTokVideoStatuses.Sent || v.Status == TikTokVideoStatuses.Failed));
        if (row is null) return 0;
        var seed = AppPromoGenerator.WeeklyPromoSeed(DateTime.UtcNow, "TikTok", row.Id);
        row.Caption = AppPromoGenerator.GeneratePromoPost("TikTok", baseUrl, seed);
        row.NarrationText = AppPromoGenerator.BuildAppVideoNarration(baseUrl, seed + 31);
        row.Title = $"BookPromoter AI — {row.WeekLabel}";
        row.CreatedAt = DateTime.UtcNow;
        row.Status = TikTokVideoStatuses.Rendering;
        row.ErrorMessage = null;
        row.VideoUrl = "";
    ...
    public bool DeleteBrandTikTokVideo(int id)
    {
        if (!IsOwner) return false;
        var uid = BrandDataUserId();
        using var db = Db();
        var row = db.TikTokVideos.FirstOrDefault(v => v.Id == id && v.UserId == uid && v.VideoKind == TikTokVideoKinds.Brand);
    ...
    public bool TikTokAutoPostEnabled
    {
        get
        {
            var user = GetCurrentUser();
            return user?.TikTokAutoPostEnabled == true;
        }
    }

    public void SetTikTokAutoPostEnabled(bool enabled)
    {
        var uid = CurrentUserId();
        if (uid == 0) return;
        using var db = Db();
        ...
        user.TikTokAutoPostEnabled = enabled;
        var schedule = db.SocialSchedules.FirstOrDefault(s =>
            s.UserId == uid
            && (s.ScheduleKind == SocialScheduleKinds.Author || s.ScheduleKind == "")
            && s.Platform.ToLower() == "tiktok");
        if (schedule is not null)
            schedule.AutoPostEnabled = enabled;
        db.SaveChanges();
        ClearUserCache();
    }

    /// <summary>
    /// Starter 2 / Professional 4 / Publisher 10 / Agency unlimited.
    /// Lifetime free + free trial use Publisher (10).
    /// </summary>
    public int? ResolveMaxTikTokVideosPerWeek(int? userId = null)
    {
        using var db = Db();
        DbUser? user;
        if (userId is int uid && uid > 0)
            user = db.Users.AsNoTracking().FirstOrDefault(u => u.Id == uid);
        else
            user = GetCurrentDbUser();
        if (user is null) return 2;

        if (user.AccessType is "Lifetime Free (Publisher)" or "Free Trial")
            return 10;
        if (user.AccessType == "Owner")
            return null;

        var planId = user.CurrentPlanId;
        if (string.IsNullOrWhiteSpace(planId)) return 2;
    ...
    public SocialSchedule EnsureTikTokScheduleForUser(int userId)
    {
        if (userId <= 0)
            return new SocialSchedule { Platform = "TikTok", PostsPerWeek = 0 };
        ...
        if (existing is not null)
        {
            ...
            var planMax = ResolveMaxTikTokVideosPerWeek(userId);
            if (planMax is int cap && existing.PostsPerWeek > cap)
            {
                existing.PostsPerWeek = cap;
                db.SaveChanges();
            }
        ...
        var max = ResolveMaxTikTokVideosPerWeek(userId);
        var defaultPosts = max ?? 7;
        var user = db.Users.AsNoTracking().FirstOrDefault(u => u.Id == userId);
        var row = new DbSocialSchedule
        {
            UserId = userId,
            ...
            RequiresApproval = false,
            AutoPostEnabled = user?.TikTokAutoPostEnabled == true,
            ScheduleKind = SocialScheduleKinds.Author,
            WeekTrackerStart = currentWeek,
            PostsSentThisWeek = 0
        };
    ...
    public SocialSchedule EnsureTikTokSchedule() =>
        EnsureTikTokScheduleForUser(CurrentUserId());

    public (string PlanLabel, int? PlanMax, int VideosPerWeek, int SentThisWeek, int GeneratedThisWeek, bool AutoPost) GetTikTokWeekQuota()
    {
        var uid = CurrentUserId();
        var schedule = EnsureTikTokScheduleForUser(uid);
        var planMax = ResolveMaxTikTokVideosPerWeek(uid);
        var (week, year, _) = AdWeek.For(DateTime.UtcNow);
        using var db = Db();
        var generated = db.TikTokVideos.Count(v =>
            v.UserId == uid && v.WeekNumber == week && v.WeekYear == year
            && v.AutoGenerated && v.Status != TikTokVideoStatuses.Removed
            && (v.VideoKind == "" || v.VideoKind == TikTokVideoKinds.Author));
        var plan = CurrentPlan;
        var planLabel = AccessType is "Lifetime Free (Publisher)" or "Free Trial"
            ? "Publisher"
            : plan?.Name ?? AccessType;
        return (planLabel, planMax, schedule.PostsPerWeek, schedule.PostsSentThisWeek, generated, TikTokAutoPostEnabled);
    }

    public string? SaveTikTokVideoSchedule(int videosPerWeek, bool autoPost)
    {
        var uid = CurrentUserId();
        if (uid == 0) return "Not logged in.";

        var planMax = ResolveMaxTikTokVideosPerWeek(uid);
        var clamped = planMax is int cap
            ? Math.Clamp(videosPerWeek, 0, cap)
            : Math.Clamp(videosPerWeek, 0, 50);

        using var db = Db();
        var user = db.Users.FirstOrDefault(u => u.Id == uid);
        if (user is null) return "User not found.";
        user.TikTokAutoPostEnabled = autoPost;
    ...
    void IncrementTikTokPostsSentThisWeek(AppDbContext db, int userId)
    {
        var currentWeek = System.Globalization.ISOWeek.GetWeekOfYear(DateTime.UtcNow);
        var schedule = db.SocialSchedules.FirstOrDefault(s =>
            s.UserId == userId
        ...
        schedule.LastPostedAt = DateTime.UtcNow;
    }

    public async Task<(bool Ok, string Message)> PostTikTokVideoAsync(
        int videoId, TikTokService tiktok, string appBaseUrl, int? userId = null, CancellationToken cancellationToken = default)
    {
        var uid = userId ?? CurrentUserId();
        if (uid == 0) return (false, "Not logged in.");

        using var db = Db();
        var dbAccount = db.SocialAccounts.FirstOrDefault(a =>
            a.UserId == uid
            && a.IsConnected
            && (a.AccountKind == SocialAccountKinds.Author || a.AccountKind == "")
            && a.Platform.ToLower() == "tiktok");
        var account = dbAccount is null ? null : ToModel(dbAccount);
        if (account is null || !account.IsLiveConnection)
            return (false, "Connect your TikTok account on the Videos page first.");

        var row = db.TikTokVideos.FirstOrDefault(v =>
            v.Id == videoId && v.UserId == uid
            && (v.VideoKind == "" || v.VideoKind == TikTokVideoKinds.Author));
        ...
        if (row.Status is TikTokVideoStatuses.Rendering or TikTokVideoStatuses.Removed)
            return (false, "Video is not ready to post yet.");

        var firstSend = row.PostedAt is null;
        if (firstSend)
        {
            var schedule = db.SocialSchedules.FirstOrDefault(s =>
                s.UserId == uid
                ...
                && s.Platform.ToLower() == "tiktok");
            var currentWeek = System.Globalization.ISOWeek.GetWeekOfYear(DateTime.UtcNow);
            if (schedule is not null && schedule.WeekTrackerStart != currentWeek)
            {
                schedule.WeekTrackerStart = currentWeek;
                schedule.PostsSentThisWeek = 0;
            }

            var allowed = schedule?.PostsPerWeek
                ?? ResolveMaxTikTokVideosPerWeek(uid)
                ?? int.MaxValue;
            var sent = schedule?.PostsSentThisWeek ?? 0;
            if (allowed <= 0)
                return (false, "Your TikTok videos/week is set to 0. Raise it on the Videos page to send to your inbox.");
            if (sent >= allowed)
                return (false, $"You've reached your TikTok limit of {allowed} video(s) this week. Upgrade your plan or wait until next week.");
        }
        ...
        {
            var caption = row.Caption.Length > 80 ? row.Caption[..80].TrimEnd() + "…" : row.Caption;
            postTitle = $"{postTitle} — {caption}";
        }

        var (result, updated, publishId) = await tiktok.SendVideoToInboxAsync(tokens, absoluteVideoUrl, postTitle, cancellationToken);
        if (updated is not null && dbAccount is not null)
        {
        ...
        if (!string.IsNullOrWhiteSpace(publishId))
            row.TikTokPublishId = publishId;

        if (result.Success)
        {
            row.Status = TikTokVideoStatuses.Sent;
            row.ErrorMessage = null;
            row.PostedAt = DateTime.UtcNow;
            if (firstSend)
                IncrementTikTokPostsSentThisWeek(db, uid);
        }
        else
        {
            // Keep downloadable Ready file; surface the post error instead of marking the render Failed.
            if (row.Status != TikTokVideoStatuses.Ready && !string.IsNullOrWhiteSpace(row.VideoUrl))
                row.Status = TikTokVideoStatuses.Ready;
    ...
    public async Task<(bool Ok, string Message)> PostBrandTikTokVideoAsync(
        int videoId, TikTokService tiktok, string appBaseUrl, CancellationToken cancellationToken = default)
    {
        ...
        var account = dbAccount is null ? null : ToModel(dbAccount);
        if (account is null || !account.IsLiveConnection)
            return (false, "Connect the BookPromoter AI TikTok account under Owner → App Videos first.");

        var row = db.TikTokVideos.FirstOrDefault(v =>
            v.Id == videoId && v.UserId == uid && v.VideoKind == TikTokVideoKinds.Brand);
        if (row is null) return (false, "Video not found.");
        if (string.IsNullOrWhiteSpace(row.VideoUrl))
            return (false, "No video file attached. Wait for rendering to finish, or regenerate.");
        if (row.Status is TikTokVideoStatuses.Rendering or TikTokVideoStatuses.Removed)
        ...
        if (firstSend)
        {
            ...
            var allowed = schedule?.PostsPerWeek ?? 2;
            var sent = schedule?.PostsSentThisWeek ?? 0;
            if (allowed <= 0)
                return (false, "Brand TikTok videos/week is set to 0. Raise it on Owner → App Videos.");
            if (sent >= allowed)
                return (false, $"You've reached this week's brand TikTok limit ({allowed}).");
        }
        ...
        if (result.Success)
        {
            ...
            row.PostedAt = DateTime.UtcNow;
            if (firstSend)
                IncrementBrandTikTokPostsSentThisWeek(db, uid);
        }
        else
        {
            if (row.Status != TikTokVideoStatuses.Ready && !string.IsNullOrWhiteSpace(row.VideoUrl))
    ...
    void IncrementBrandTikTokPostsSentThisWeek(AppDbContext db, int userId)
    {
        var currentWeek = System.Globalization.ISOWeek.GetWeekOfYear(DateTime.UtcNow);
        var schedule = db.SocialSchedules.FirstOrDefault(s =>
            s.UserId == userId
    ...
    /// <summary>Send Ready videos to TikTok inbox for users who enabled auto-post.</summary>
    public async Task<int> AutoPostReadyTikTokVideosAsync(
        TikTokService tiktok, string appBaseUrl, CancellationToken cancellationToken = default)
    {
        using var db = Db();
        var userIds = await db.Users
            .Where(u => u.TikTokAutoPostEnabled && u.HasCustomerAccess)
            .Select(u => u.Id)
            .ToListAsync(cancellationToken);
        ...
        {
            var hasLiveTikTok = await db.SocialAccounts.AnyAsync(a =>
                a.UserId == uid
                && a.IsConnected
                && (a.AccountKind == SocialAccountKinds.Author || a.AccountKind == "")
                && a.Platform.ToLower() == "tiktok"
                && a.AccessToken != null
                && a.AccessToken != ""
                && a.ExternalAccountId != null
                && a.ExternalAccountId != "", cancellationToken);
            if (!hasLiveTikTok) continue;

            var schedule = EnsureTikTokScheduleForUser(uid);
            if (schedule.PostsPerWeek <= 0) continue;
            var remaining = Math.Max(0, schedule.PostsPerWeek - schedule.PostsSentThisWeek);
            if (remaining == 0) continue;

            var readyIds = await db.TikTokVideos
                .Where(v => v.UserId == uid
                    && (v.VideoKind == "" || v.VideoKind == TikTokVideoKinds.Author)
                    && v.Status == TikTokVideoStatuses.Ready
                    && v.VideoUrl != ""
                    && v.PostedAt == null)
                .OrderBy(v => v.CreatedAt)
                .Select(v => v.Id)
            ...
            foreach (var videoId in readyIds)
            {
                cancellationToken.ThrowIfCancellationRequested();
                var (ok, _) = await PostTikTokVideoAsync(videoId, tiktok, appBaseUrl, uid, cancellationToken);
                if (ok) posted++;
            }
        }
    ...
    public async Task<int> AutoPostReadyBrandTikTokVideosAsync(
        TikTokService tiktok, string appBaseUrl, CancellationToken cancellationToken = default)
    {
        var uid = BrandDataUserId();
        if (uid == 0) return 0;
        ...
        var schedule = EnsureBrandTikTokScheduleForUser(uid);
        if (!schedule.AutoPostEnabled || schedule.PostsPerWeek <= 0) return 0;
        var remaining = Math.Max(0, schedule.PostsPerWeek - schedule.PostsSentThisWeek);
        if (remaining == 0) return 0;

        using var db = Db();
        var hasLive = await db.SocialAccounts.AnyAsync(a =>
            a.UserId == uid
            && a.IsConnected
            && a.AccountKind == SocialAccountKinds.Brand
            && a.Platform.ToLower() == "tiktok"
            && a.AccessToken != null && a.AccessToken != ""
            && a.ExternalAccountId != null && a.ExternalAccountId != "", cancellationToken);
        if (!hasLive) return 0;

        var readyIds = await db.TikTokVideos
            .Where(v => v.UserId == uid
                && v.VideoKind == TikTokVideoKinds.Brand
                && v.Status == TikTokVideoStatuses.Ready
                && v.VideoUrl != ""
                && v.PostedAt == null)
            .OrderBy(v => v.CreatedAt)
        ...
        var posted = 0;
        // PostBrandTikTokVideoAsync requires IsOwner session — use inline brand path when scheduler.
        foreach (var videoId in readyIds)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var (ok, _) = await PostBrandTikTokVideoInternalAsync(db, videoId, uid, tiktok, appBaseUrl, cancellationToken);
            if (ok) posted++;
        }
    ...
    async Task<(bool Ok, string Message)> PostBrandTikTokVideoInternalAsync(
        AppDbContext db, int videoId, int uid, TikTokService tiktok, string appBaseUrl, CancellationToken cancellationToken)
    {
        var dbAccount = db.SocialAccounts.FirstOrDefault(a =>
            a.UserId == uid
            && a.IsConnected
        ...
        var account = dbAccount is null ? null : ToModel(dbAccount);
        if (account is null || !account.IsLiveConnection)
            return (false, "Brand TikTok not connected.");

        var row = db.TikTokVideos.FirstOrDefault(v =>
            v.Id == videoId && v.UserId == uid && v.VideoKind == TikTokVideoKinds.Brand);
        if (row is null || string.IsNullOrWhiteSpace(row.VideoUrl)) return (false, "Video not ready.");
        if (row.Status is TikTokVideoStatuses.Rendering or TikTokVideoStatuses.Removed)
        ...
        {
            var schedule = db.SocialSchedules.FirstOrDefault(s =>
                s.UserId == uid && s.ScheduleKind == SocialScheduleKinds.Brand && s.Platform.ToLower() == "tiktok");
            var currentWeek = System.Globalization.ISOWeek.GetWeekOfYear(DateTime.UtcNow);
            if (schedule is not null && schedule.WeekTrackerStart != currentWeek)
            {
                schedule.WeekTrackerStart = currentWeek;
            ...
            if (schedule is not null && schedule.PostsSentThisWeek >= schedule.PostsPerWeek)
                return (false, "Week limit reached.");
        }
        ...
        else if (row.Status != TikTokVideoStatuses.Ready)
        {
            row.Status = TikTokVideoStatuses.Ready;
            row.ErrorMessage = result.Message;
        }
    ...
    public bool DeleteTikTokVideo(int id)
    {
        var uid = CurrentUserId();
    ...
    public void EnsureCurrentWeekPostSlots()
    {
        ...
        foreach (var schedule in schedules)
        {
            var platformAds = weekAds
                ...
                .ToList();
            if (platformAds.Count == 0) continue;
            PostSchedule.AssignWeeklyPostSlots(platformAds, schedule.PostsPerWeek, now, currentYear, currentWeek, onlyAdsMissingFutureSlot: true);
            changed = true;
        }
    ...
    public Dictionary<string, int> OwnerBookPromoClicksByPlatform()
    {
        if (!IsOwner) return [];
        using var db = Db();
        var totals = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
        foreach (var json in db.Books.AsNoTracking().Select(b => b.PlatformClickHistoryJson))
        {
            foreach (var month in ParsePlatformClickHistory(json).Values)
            {
                foreach (var (platform, clicks) in month)
                    totals[platform] = totals.GetValueOrDefault(platform) + clicks;
            }
        }
    ...
    public async Task RefreshOwnerBrandPostMetricsAsync(SocialPostMetricsService metrics, CancellationToken cancellationToken = default)
    {
        if (!IsOwner) return;
        using var db = Db();
        var owner = await db.Users.AsNoTracking().FirstOrDefaultAsync(u => u.Email == OwnerAccount.NormalizedEmail, cancellationToken);
        if (owner is null) return;

        var logs = await db.PostingLog
            .Where(l => l.UserId == owner.Id
                && l.LogKind == PostingLogKinds.Brand
                && l.Success
                && l.ExternalPostId != null
                && l.ExternalPostId != "")
            .OrderByDescending(l => l.AttemptedAt)
            .Take(25)
            .ToListAsync(cancellationToken);
        ...
        {
            var account = accounts.FirstOrDefault(a => PostLimits.PlatformsMatch(a.Platform, log.Platform));
            if (account is null) continue;
            if (await metrics.RefreshAsync(log, ToModel(account), cancellationToken))
                changed = true;
        }
    ...
    static DbPostingLogEntry NewPostingLogEntry(
        int userId,
        string platform,
        string bookTitle,
        bool success,
    ...
    public async Task<int> RunDuePostsAsync(SocialPostingService postingService, int? userId = null)
    {
        ...
        foreach (var schedule in schedules)
        {
            ...
            db.PostingLog.Add(NewPostingLogEntry(schedule.UserId, schedule.Platform, candidate.BookTitle, posted, result.Message, now, PostingLogKinds.Author, candidate.Id, result, PostDeliveryKinds.Scheduled));
    ...
    public async Task<(bool Success, string Message)> PostAdNowAsync(int adId, SocialPostingService postingService)
    {
        ...
        db.PostingLog.Add(NewPostingLogEntry(uid, ad.Platform, ad.BookTitle, posted, posted ? "Posted now from Ad Library." : result.Message, now, PostingLogKinds.Author, ad.Id, result, PostDeliveryKinds.Manual));
    ...
    static bool ApplyGeneratedAdPostResult(DbGeneratedAd ad, PostingResult result, DateTime now, string? postDelivery = null)
    {
        if (!string.IsNullOrWhiteSpace(postDelivery))
            ad.PostedVia = postDelivery;
        ...
        if (postToSocial)
        {
            ...
            if (owner is not null)
            {
                ...
                foreach (var account in accounts)
                {
                    ...
                    db.PostingLog.Add(NewPostingLogEntry(owner.Id, account.Platform, $"Update v{version}", result.PostedToFeed, result.Message, DateTime.UtcNow, PostingLogKinds.Brand, result: result, postDelivery: PostDeliveryKinds.Manual));
    ...
    static TikTokVideo ToModel(DbTikTokVideo v) => new()
    {
        ...
        VideoKind = string.IsNullOrWhiteSpace(v.VideoKind) ? TikTokVideoKinds.Author : v.VideoKind,
    ...
    static PostingLogEntry ToModel(DbPostingLogEntry l) => new()
    {
        ...
        MetricsFetchedAt = l.MetricsFetchedAt,
        PostDelivery = l.PostDelivery
    };
