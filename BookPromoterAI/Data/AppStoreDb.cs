...
class AppStoreDb
{
    ...
    public int EnsureWeeklyVideos(PostGenerator generator, string baseUrl, int? userId = null)
    {
        ...
        var existingBookIds = db.TikTokVideos
            .Where(v => v.UserId == uid && v.WeekNumber == currentWeek && v.WeekYear == currentYear && v.AutoGenerated)
            .Select(v => v.BookId)
            .ToHashSet();
    ...
    public int RetryFailedWeeklyVideo(int id, PostGenerator generator)
    {
        var uid = CurrentUserId();
        if (uid == 0) return 0;
        using var db = Db();
        var row = db.TikTokVideos.FirstOrDefault(v =>
            v.Id == id && v.UserId == uid && v.AutoGenerated && v.Status == TikTokVideoStatuses.Failed);
        if (row is null) return 0;

        var book = db.Books.AsNoTracking().FirstOrDefault(b => b.Id == row.BookId && b.UserId == uid);
        if (book is not null)
